                ...
                try {
                    ...
                    if (rq.equals("log out")) {
                        ...
                        synchronized(lock){
                            ...
                            clientMap.get(u.getUsername()).getOos().writeUTF("bxhScore");
                            clientMap.get(u.getUsername()).getOos().writeObject(list);
                            clientMap.get(u.getUsername()).getOos().flush();
                        }
                    }
...
//                            System.out.println(list.size());
                            clientMap.get(u.getUsername()).getOos().writeUTF("bxhTime");
                            clientMap.get(u.getUsername()).getOos().writeObject(list);
                            clientMap.get(u.getUsername()).getOos().flush();
                        }
...
//                            clientMap.get(thisu.getUsername()).getOos().writeUTF("challenge");
                            clientMap.get(u2.getUsername()).getOos().writeUTF("challenge");
                            clientMap.get(u2.getUsername()).getOos().writeObject(u2);
                            clientMap.get(u2.getUsername()).getOos().writeObject(u1);
                        }
//                        updateOnlineUsers();
                    } 
...
//                        }
                        ...
                        synchronized(lock){
                            ...
                            Users u2 = (Users) ois.readObject();
                            Users u1 = (Users) ois.readObject();
                            clientMap.get(u2.getUsername()).getOos().writeUTF("begin");
                            clientMap.get(u2.getUsername()).getOos().writeObject(u2);
                            clientMap.get(u2.getUsername()).getOos().writeObject(a);
                            clientMap.get(u2.getUsername()).getOos().writeObject(value2);
                            clientMap.get(u1.getUsername()).getOos().writeUTF("begin");
                            clientMap.get(u1.getUsername()).getOos().writeObject(u1);
                            clientMap.get(u1.getUsername()).getOos().writeObject(a);
                            clientMap.get(u1.getUsername()).getOos().writeObject(value2);
                            pairs.add(new Pair<>(clientMap.get(u1.getUsername()),clientMap.get(u2.getUsername())));
...
//                            }
                            if(temp_pair.getKey().getUser().getCheck()==1&&temp_pair.getValue().getUser().getCheck()==1){
                                ...
                                for (int i=0;i<4;i++) {
                                    if(Integer.parseInt(arStr[i])> Integer.parseInt(arStr2[i])){
                                        ...
                                        int t =db.insertResult(temp_pair.getValue().getUser(),temp_pair.getKey().getUser());
                                        ...
                                        if(Integer.parseInt(arStr[i])<Integer.parseInt(arStr2[i])){
                                            ...
                                            db.updatePoints(temp_pair.getKey().getUser(), 1);
                                            int t=db.insertResult(temp_pair.getKey().getUser(),temp_pair.getValue().getUser());
                                            System.out.println(t);
                                            break;
                                        }
                                    }
                        ...
                        synchronized(lock){
                            ...
                            if(temp_pair.getKey().getUser().getCheck()==2&&temp_pair.getValue().getUser().getCheck()==2){
                                temp_pair.getKey().oos.writeUTF("begin");
                                temp_pair.getKey().oos.writeObject(temp_pair.getKey().getUser());
                                temp_pair.getKey().oos.writeObject(a);
                                temp_pair.getKey().oos.writeObject(value2);
                                temp_pair.getValue().oos.writeUTF("begin");
                                temp_pair.getValue().oos.writeObject(temp_pair.getValue().getUser());
                                temp_pair.getValue().oos.writeObject(a);
                                temp_pair.getValue().oos.writeObject(value2);
                                temp_pair.getKey().getUser().setCheck(0);
                                temp_pair.getValue().getUser().setCheck(0);
                            }
                            if(temp_pair.getKey().getUser().getCheck()==3){
                                db.updateStatus(temp_pair.getValue().getUser(),1);
                                updateOnlineUsers();
                            }else if(temp_pair.getValue().getUser().getCheck()==3){
